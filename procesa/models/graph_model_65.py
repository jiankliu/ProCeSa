...
class GraphModel65(BaseModel):
    def __init__(self,
            in_features=1152,
            hidden_features=1024,
            num_classes=2,  # add num_classes for classification task
            # output_features=1,  # delete regression variable
            attention_features=128,
            attention_heads=4,
            loss_reg=None,
            loss_cls=dict(type='CrossEntropyLoss'),  # add loss_cls for classification task
            loss_triplet=dict(type='TripletLoss'),                 
            train_cfg=None,
            test_cfg=None):
        super(GraphModel65, self).__init__()
        ...
        self.fc_final = nn.Linear(hidden_features*1, num_classes)
    ...
    def forward_train(self, graphs, labels, sequences):
        ...
        losses = dict()
        loss_cls = self.loss_cls(output, labels)
