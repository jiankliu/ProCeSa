...
def get_loader(
    ...
    return DataLoader(
            ...
            collate_fn=lambda samples: collate_fn(samples, scale_label=scale_label),
            pin_memory=True)


def collate_fn(samples, scale_label=False):
    ...
    if seq_num[0] is not None:
        seq_num = torch.as_tensor(list(itertools.chain.from_iterable(seq_num)), dtype=torch.long)
    return names, sequences, graphs, labels, seq_num
...
class ProteinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(os.path.join(pklroot, f"{names}.pickle"), 'rb') as f:
            valid_names, valid_sequences, valid_graphs = joblib.load(f)
