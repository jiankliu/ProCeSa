...
def loop_val(data_loader, model, epoch, logger, task='reg'):
    y_true, y_pred = [], []
    model.eval()
    for i, batch in enumerate(data_loader):
    ...
    if task == 'cls':
        results = cal_metric_cls(y_true, y_pred)
    elif task == 'reg':
        results = cal_metric(y_true, y_pred, threshold=0.5)
    else:
        raise ValueError("task should be 'cls' or 'reg'")

    logger.info("Val | Epoch: {} | {}".format(epoch, {k:"{:.3f}".format(v) for k, v in results.items()}))

    return results

def cal_metric_cls(y_true, y_pred):
    # calculate micro, macro, weighted accuracy, precision, recall, f1
    y_true, y_pred = np.array(y_true), np.array(y_pred)
    y_pred = np.argmax(y_pred, axis=1)
    acc = metrics.accuracy_score(y_true, y_pred)
    prec_w = metrics.precision_score(y_true, y_pred, average='weighted')
    rec_w = metrics.recall_score(y_true, y_pred, average='weighted')
    f1_w = metrics.f1_score(y_true, y_pred, average='weighted')
    prec_ma = metrics.precision_score(y_true, y_pred, average='macro')
    rec_ma = metrics.recall_score(y_true, y_pred, average='macro')
    f1_ma = metrics.f1_score(y_true, y_pred, average='macro')
    prec_mi = metrics.precision_score(y_true, y_pred, average='micro')
    rec_mi = metrics.recall_score(y_true, y_pred, average='micro')
    f1_mi = metrics.f1_score(y_true, y_pred, average='micro')
    return {'acc': acc, 'prec-w': prec_w, 'rec-w': rec_w, 'f1-w': f1_w,
            'prec-ma': prec_ma, 'rec-ma': rec_ma, 'f1-ma': f1_ma,
            'prec-mi': prec_mi, 'rec-mi': rec_mi, 'f1-mi': f1_mi}
